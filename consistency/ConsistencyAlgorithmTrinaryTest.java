				...
				for(int c = 0; c < N; c++) {
					if(a < b && b < c) {
						for(int nt = 1; nt <= N / 2; nt++) {
							...
							final ConsistencyProblem consistencyProblem = new ConsistencyProblem(nt, constraints, N);
							...
							for(long bits = 0; bits < (1L << N); bits++) {
								...
								final boolean[] booleanVector = Utility.toBooleans(vector);
								testAssertEquals(
									consistencyState.isValidSolution(vector),
									constraintABC.matches(booleanVector) && numberOfTruesInVector == nt,
					...
					for(int d = 0; d < N; d++) {
						if(a <= b && c <= d) {
							for(int nt = 1; nt <= N / 2; nt++) {
								...
								final ConsistencyProblem problem = new ConsistencyProblem(nt, constraints, N);
								final ConsistencyState consistencyState = new ConsistencyState(problem);
								consistencyState.solve();
								...
								for(long bits = 0; bits < (1L << N); bits++) {
									...
									testAssertEquals(
										consistencyState.isValidSolution(vector),
										constraintAB.matches(booleanVector) && constraintCD.matches(booleanVector) && nt == numberOfTruesInVector,
