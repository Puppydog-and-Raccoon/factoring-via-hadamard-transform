			...
			for(int b = 0; b < N; b++) {
				if(a < b) {
					for(int numberOfTruesInProblem = 0; numberOfTruesInProblem <= N / 2; numberOfTruesInProblem++) {
						...
						final EquationButterfly equationButterfly = new EquationButterfly(positionButterfly, constraint, numberOfTruesInProblem);
						equationButterfly.fill();

						for(long bits = 0; bits < (1L << N); bits++) {
							testAssertEquals(
								equationButterfly.isValid(Utility.toInts(bits, N)),
